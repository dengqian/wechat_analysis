...
class stat_struct:
    ...
    def update_state(self, pd, pkt):
        ...
        if pd.dir == CTOS:
            ...
            if self.s_seq_base == 0:
                self.s_seq_base = pd.ack - 1
                self.c_ack_base = pd.ack - 1
            # self.ctos_list.append((pd.seq-self.c_seq_base, pd.ack-self.c_ack_base, pd.leng))
            ...
            if pd.ack in self.stoc_time_dict:
            ...
            if pd.leng > 0:
                ...
                self.ctos_pkt_cnt += 1 
                if pd.seq-self.c_seq_base>1 and pd.seq-self.c_seq_base <= self.c_seq_sent:
        ...
        if pd.dir == STOC:
            ...
            if self.c_seq_base == 0:
                self.c_seq_base = pd.ack - 1
                self.s_ack_base = pd.ack - 1
            # self.stoc_list.append((pd.seq-self.s_seq_base, pd.ack-self.s_ack_base, pd.leng))
            if self.tcp_state == TCP_SYN_RCVD and pd.seq == self.s_seq_base+1\
                    and pd.ack == self.s_ack_base+1:
                if self.syn_pkt_cnt == 2:
                    self.syn_rtt = pd.ts - self.time_base 
                self.tcp_state = TCP_ESTABLISHED
            if pd.ack in self.ctos_time_dict:
            ...
            if pd.leng > 0:
                ...
                self.stoc_pkt_cnt += 1
                if pd.seq-self.s_seq_base>1 and pd.seq-self.s_seq_base <= self.s_seq_sent:
    ...
    def print_conn_info(self):
        ...
        if len(self.ctos_rtt_list) > 0:
            max_ctos_rtt = max(self.ctos_rtt_list)
            # avg_ctos_rtt = numpy.mean(self.ctos_rtt_list)
            sum = 0
            for it in self.ctos_rtt_list:
                sum += it
            avg_ctos_rtt = sum/len(self.ctos_rtt_list) 
        if len(self.stoc_rtt_list) > 0:
            max_stoc_rtt = max(self.stoc_rtt_list)
            # avg_stoc_rtt = numpy.mean(self.stoc_rtt_list)
            sm = 0
            for it in self.stoc_rtt_list:
                sum += it
            avg_stoc_rtt = sum/len(self.stoc_rtt_list) 

        stoc_retrans_rate = 0
        ctos_retrans_rate = 0
        if self.stoc_retrans_cnt != 0:
            stoc_retrans_rate = self.stoc_retrans_cnt*1.0/self.s_seq_sent
        if self.ctos_retrans_cnt != 0:
            ctos_retrans_rate = self.ctos_retrans_cnt*1.0/self.c_seq_sent
        if self.s_seq_sent+self.c_seq_sent > 0:
            self.retrans_rate = (self.stoc_retrans_cnt+self.ctos_retrans_cnt)/\
                    (self.s_seq_sent+self.c_seq_sent+self.stoc_retrans_cnt+self.ctos_retrans_cnt) 
        ...
        if self.stream_type == 'needed':
            if self.ctos_pkt_cnt + self.stoc_pkt_cnt > 1:
                print self.key, self.date, self.weixinnum_flag, self.rst_flag, self.rst_dir,\
                    "%.4f" % (self.finish_time-self.time_base), self.data_dir,\
                    "%.4f" % self.retrans_rate,"%.4f" % ctos_retrans_rate, "%.4f" \
                    % stoc_retrans_rate,"%.4f" % self.syn_rtt, "%.4f" % max_ctos_rtt,\
                    "%.4f" % avg_ctos_rtt,"%.4f" % max_stoc_rtt, "%.4f" % avg_stoc_rtt,\
                    self.fin_flag, max(self.c_ack_sent, self.s_ack_sent), self.content_length, self.total_size
