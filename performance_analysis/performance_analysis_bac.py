...
__ts = [ 'TCP_LISTEN', 'TCP_SYN_RCVD', 'TCP_SYN_SENT', \
        'TCP_ESTABLISHED', 'LAST_ACK', 'TCP_FIN_1', \
...
class stat_struct:
    ...
    def update_state(self, pd, pkt):
        ...
        if pd.dir == CTOS:
            ...
            if self.tcp_state == TCP_SERVER_CLOSED:
                if pd.ack == self.s_seq_sent + 1 or pd.ack == self.c_seq_sent + 1:
                    self.tcp_state = TCP_CLOSED

            if pd.flag == 'F':
                if self.tcp_state == TCP_FIN_RCVD:
                    self.tcp_state = LAST_ACK
                else:
    ...
    def print_conn_info(self):
        self.finish_time = self.finish_time-self.time_base+self.syn_rtt
        retrans_rate = 0
